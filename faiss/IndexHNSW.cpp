...
void hnsw_add_vertices(
    ...
    size_t ntotal = n0 + n;
    // std::cout << "here in hnsw_add_vertices" << std::endl;
    // print out ntotal, n0, n, x[0]
    ...
    // std::cout << "here 2 in hnsw_add_vertices" << std::endl;
    ...
    { // make buckets with vectors of the same level
        ...
        // std::cout << "here 4 in hnsw_add_vertices" << std::endl;
        // accumulate
        ...
        // std::cout << "here 5 in hnsw_add_vertices" << std::endl;
        // bucket sort
        for (int i = 0; i < n; i++) {
            storage_idx_t pt_id = i + n0;
            int pt_level = hnsw.levels[pt_id] - 1;
    ...
    // std::cout << "here 6 in hnsw_add_vertices" << std::endl;
    ...
    { // perform add
        ...
        for (int pt_level = hist.size() - 1; pt_level >= 0; pt_level--) {
            ...
            // std::cout << "pt_level: " << pt_level << std::endl;
...
#pragma omp for schedule(static)
                for (int i = i0; i < i1; i++) {
                    ...
                    // std::cout << "here 7 in hnsw_add_vertices" << std::endl;
                    hnsw.add_with_locks(*dis, pt_level, pt_id, locks, vt);
                    // std::cout << "here 8 in hnsw_add_vertices" << std::endl;
                    if (prev_display >= 0 && i - i0 > prev_display + 10000) {
                        prev_display = i - i0;
                        printf("  %d / %d\r", i - i0, i1 - i0);
                        fflush(stdout);
                    }
                    // std::cout << "here 9 in hnsw_add_vertices" << std::endl;
                    if (counter % check_period == 0) {
                        if (InterruptCallback::is_interrupted()) {
                            interrupt = true;
                        }
                    }
                    // std::cout << "here 10 in hnsw_add_vertices" << std::endl;
                    counter++;
                    // std::cout << "here 11 in hnsw_add_vertices" << std::endl;
                }
                // std::cout << "here 12 in hnsw_add_vertices" << std::endl;
            }
            // std::cout << "here 12.5 in hnsw_add_vertices" << std::endl;
            if (interrupt) {
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
    ...
    // printf("search: n1: %zd, n2: %zd, n3: %zd, ndis: %zd, nreorder: %zd\n",
    //        n1,
    //        n2,
    //        n3,
    //        ndis,
...
void IndexHNSW::partition_search(
    ...
    // int efSearch = hnsw.efSearch;
    int efSearch = partition_indices[0]->hnsw.efSearch;
    if (params_in) {
        params = dynamic_cast<const SearchParametersHNSW*>(params_in);
        FAISS_THROW_IF_NOT_MSG(params, "params type invalid");
        efSearch = params->efSearch;
        for (int i = 0; i < num_partitions; i++) {
            partition_indices[i]->hnsw.efSearch = efSearch;
        }
    }
    size_t n1 = 0, n2 = 0, n3 = 0, ndis = 0, nreorder = 0;
...
void IndexHNSW::add(idx_t n, const float* x) {
    ...
    // std::cout << "here in add 2" << std::endl;
    storage->add(n, x);

    // std::cout << "here in add 3" << std::endl;
...
// added for debugging
void IndexHNSW::printStats(bool print_edge_list, bool is_hybrid) {
    hnsw.print_neighbor_stats(print_edge_list, is_hybrid);
    printf("METADATA VEC for number nodes per level\n");
    for (int i = 0; i < hnsw.nb_per_level.size(); i++) {
        printf("\tlevel %d: %d nodes\n", i, hnsw.nb_per_level[i]);
    }
}
